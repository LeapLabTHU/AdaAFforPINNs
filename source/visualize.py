...
def u_diff(X_f_train, Exact, U_pred, x, t, nu, beta, rho, seed, layers, N_f, L, source, lr, u0_str, system, path, relative_error = False):
    ...
    # ax.scatter(X_f_train[:int(0.9*N_f), 1], X_f_train[:int(0.9*N_f), 0])
    # ax.scatter(X_f_train[int(0.9*N_f):, 1], X_f_train[int(0.9*N_f):, 0], color='red')
    line = np.linspace(x.min(), x.max(), 2)[:,None]
...
def u_predict(u_vals, U_pred, x, t, nu, beta, rho, seed, layers, N_f, L, source, lr, u0_str, system, path):
    ...
    h = ax.imshow(U_pred.T, interpolation='nearest', cmap='rainbow',
                  extent=[t.min(), t.max(), x.min(), x.max()],
                  origin='lower', aspect='auto', vmin=u_vals.min(), vmax=u_vals.max())
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="5%", pad=0.10)
    cbar = fig.colorbar(h, cax=cax)
    cbar.ax.tick_params(labelsize=15)
    ...
    plt.savefig(f"{path}/upredicted_{system}_nu{nu}_beta{beta}_rho{rho}_Nf{N_f}_{layers}_L{L}_seed{seed}_source{source}_{u0_str}_lr{lr}.png")
